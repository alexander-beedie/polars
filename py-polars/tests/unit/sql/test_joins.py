...
def test_join_with_literal_string_comparison() -> None:
    df1 = pl.DataFrame(  # noqa: F841
        {
...
def test_join_with_expression_conditions(expression: str, expected_length: int) -> None:
    df1 = pl.DataFrame(  # noqa: F841
        {
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            pl.DataFrame({"a": [10, 20, 30], "b": [5, 10, 15]}),
            pl.DataFrame({"c": [15, 30, 45], "d": [3, 6, 9]}),
            "df1.a + df1.b = df2.c",
            "df1.a, df1.b, df2.c, df2.d",
            [(10, 5, 15, 3), (20, 10, 30, 6), (30, 15, 45, 9)],
            ["a", "b", "c", "d"],
        ),
        (
            pl.DataFrame({"x": [5, 10, 15]}),
...
def test_join_expression_arithmetic(
    df1: pl.DataFrame,
    df2: pl.DataFrame,
    join_constraint: str,
    select_cols: str,
...
def test_join_reversed_constraint_order(
    df1: pl.DataFrame,
    df2: pl.DataFrame,
    join_constraint: str,
...
def test_join_mixed_constraints(
    df1: pl.DataFrame,
    df2: pl.DataFrame,
    join_constraint: str,
    select_cols: str,
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            "LOWER(LTRIM(RTRIM(df1.text))) = df2.text",
...
def test_join_nested_function_expressions(
    df1: pl.DataFrame,
    df2: pl.DataFrame,
    join_constraint: str,
    expected: list[tuple[Any, ...]],
...
def test_join_expression_with_literals(
    df1: pl.DataFrame,
    df2: pl.DataFrame,
    join_constraint: str,
    select_cols: str,
...
def test_join_unqualified_expressions(
    df1: pl.DataFrame,
    df2: pl.DataFrame,
    join_constraint: str,
    expected: list[tuple[Any, ...]],
