...
fn apply_float_rounding_op(
    ...
    if let Ok(ca) = s.f32() {
        return Some(if decimals == 0 {
            ...
            // Do the computation on f64 floats to not lose precision;
...
pub trait RoundSeries: SeriesSealed {
    ...
    fn round(&self, decimals: u32, mode: RoundMode) -> PolarsResult<Series> {
        ...
        let (f32_op, f64_op) = match mode {
            RoundMode::HalfToEven => (f32::round_ties_even, f64::round_ties_even),
            RoundMode::HalfAwayFromZero => (f32::round, f64::round),
        };
        // try float types first
        if let Some(result) = apply_float_rounding_op(s, decimals, f32_op, f64_op) {
            return result;
        }
    ...
    fn truncate(&self, decimals: u32) -> PolarsResult<Series> {
        let s = self.as_series();
        if let Some(result) = apply_float_rounding_op(s, decimals, f32::trunc, f64::trunc) {
            return result;
        }
