    ...
    pub fn new(kind: CumAggKind, dtype: &DataType) -> Self {
        let state = match kind {
            ...
            CumAggKind::Mean if matches!(dtype, DataType::Decimal(_, _)) => {
                CumAggState::MeanDecimal { sum: 0, count: 0 }
            },
            CumAggKind::Mean => CumAggState::Mean { sum: 0.0, count: 0 },
        ...
        join_handles.push(scope.spawn_task(TaskPriority::High, async move {
            while let Ok(mut m) = recv.recv().await {
                ...
                let out = match (&mut self.state, self.kind) {
                    #[cfg(feature = "dtype-decimal")]
                    (CumAggState::MeanDecimal { sum, count }, CumAggKind::Mean) => {
                    ...
                    #[cfg(feature = "dtype-decimal")]
                    (CumAggState::Mean { .. }, CumAggKind::Mean)
                        if matches!(s.dtype(), DataType::Decimal(_, _)) =>
                    {
                        polars_bail!(InvalidOperation: "incorrect state type for Decimal; expected CumAggState::MeanDecimal")
                    },
                    (CumAggState::Mean { sum, count }, CumAggKind::Mean) => {
                        let (out, new_sum, new_count) =
